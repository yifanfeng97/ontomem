...
- **[2026-01-28] v0.2.0 发布**:
  - **✨ Lookups 功能**: O(1) 多维精确匹配查询，合并时自动维护索引一致性
  - **🔍 二级索引**: 在任何数据字段上创建自定义索引，无需向量搜索的开销
  - **🔄 自动同步**: 当条目合并时，索引自动更新，保持完美一致性
  - **🎯 复合键**: 支持复杂索引策略（复合键、分层键等）
  - [了解更多 →](user-guide/lookups.md)
