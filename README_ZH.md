...
- **[2026-09-05] 🎉 v0.4.0：带出处的记忆（来源账本）**：
  ...
  - **可选账本**：`track_sources=True`；约 1.5-2 倍存储开销且有界（每个来源仅保留当前版本）
  - **`suspended_index()`**：批量变更期间不丢弃向量索引
  - **Embedder 指纹**：索引记录 embedder 签名，拒绝混用不同向量空间

- **[2026-09-04] 🎉 v0.3.0：增量索引维护与语义编辑**：
  - **`sync_index()`**：原地修补 FAISS 索引——只删除/重嵌受影响的向量，而非丢弃整个索引（小幅编辑最多可减少 200 倍 embedding 调用）
  - **`remove_many()` / `upsert()`**：批量删除与按键替换语义，保全索引
  - **`edit()`**：大模型辅助的单条目语义编辑——移除一条错误事实、保留其余内容，带 key 不变性校验、dry-run 与向量原地刷新
  - **`index_built`**：`has_index()` 的 property 别名
